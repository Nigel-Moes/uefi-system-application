...
// Function to get the current CPU exception level
UINTN GetEL() {
	UINTN el;
	...
	) 
{
	...
	// Print the current CPU exception level
	Print(L"Current CPU exception level: %u\n", GetEL());

	// Retrieve the array of all handles in the handle database and place them in a handle buffer
	Status = gBS->LocateHandleBuffer(AllHandles, NULL, NULL, &HandleCount, &HandleBuffer);
	if (EFI_ERROR(Status)) { Print(L"Failed to locate handles: %r\n", Status); return Status; }
